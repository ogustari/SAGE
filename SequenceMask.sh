...
echo "Add a second cut position? (Y or N followed by [ENTER])"
read Answer1
if [[ "$Answer1" == 'Y' ]]; then
  # cut position no 2
...
then
  ...
  filenoextension=$(echo $readsfile | cut -f 1 -d '.')
  cat $readsfile | grep '^>' > tmp_header.fasta
  cat tmp_header.fasta tmp_mask3.fasta > $filenoextension'_mask.fa'
elif [[ ($Answer1 == 'Y') && ($Answer2 == 'N') ]]
then
  # create cut position file
  echo "creating cut position file..."
  touch tmp_cut_start_position.txt
  ...
  # sort cut position variables
  Start1=$(cat tmp_cut_sort_position.txt | awk 'NR==2 {print $1}')
  End1=$(cat tmp_cut_sort_position.txt | awk 'NR==2 {print $2}')
  Start2=$(cat tmp_cut_sort_position.txt | awk 'NR==1 {print $1}')
  End2=$(cat tmp_cut_sort_position.txt | awk 'NR==1 {print $2}')
  ...
  filenoextension=$(echo $readsfile | cut -f 1 -d '.')
  cat $readsfile | grep '^>' > tmp_header.fasta
  cat tmp_header.fasta tmp_mask2.fasta > $filenoextension'_mask.fa'
else
  # prepare file to be cut
  echo "file preparation..."
  cat $readsfile | grep -v '^>' | sed ':a;N;$!ba;s/\n//g' > tmp_NoHeader_Noline.fasta
  ...
  # cut sequence(s) and masking first and last position
  echo "Cuting out sequence(s)..."
  cat tmp_NoHeader_Noline.fasta | awk -v var1="$PosCut1Start" -v var2="$PosCut1End" '{print substr($0,1,var1) substr($0,var2)}' > tmp_Cut1.fasta
  echo "Masking first and last position..."
  Count1=1
  Count2=2
  MaskStart1=$(($PosCut1Start-$Count1))
  MaskEnd1=$(($PosCut1Start+$Count2))
  cat tmp_Cut1.fasta | awk -v var7="$MaskStart1" -v var8="$MaskEnd1" '{print substr($0,1,var7) "XX" substr($0,var8)}' > tmp_mask1.fasta
  ...
  filenoextension=$(echo $readsfile | cut -f 1 -d '.')
  cat $readsfile | grep '^>' > tmp_header.fasta
  cat tmp_header.fasta tmp_mask1.fasta > $filenoextension'_mask.fa'
fi
